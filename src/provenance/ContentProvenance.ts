...
export class ContentProvenance {
    ...
    constructor(
        ...
        generateId: () => string = () => `prov-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    ) {
        this.config = config;
    ...
    async getChain(targetId: string): Promise<ProvenanceChain> {
        ...
        const totalAiContribution = records.reduce(
            (sum, r) => sum + (r.aiContribution ?? 0),
            0,
        ) / records.length;
        ...
        for (const record of records) {
            const valid = await this.config.verify(record.ucanToken, record.contentHash);
            if (!valid) {
                integrity = 'broken';
                break;
            }
