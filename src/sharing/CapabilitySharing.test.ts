...
describe('CapabilitySharing', () => {
    ...
    describe('claiming links', () => {
        ...
        it('returns null for nonexistent link', async () => {
            const grant = await sharing.claimShareLink('fake-uri', 'did:test:claimer');
            expect(grant).toBeNull();
        });
        ...
        it('enforces DID-specific links', async () => {
            ...
            const wrongUser = await sharing.claimShareLink(link.uri, 'did:test:wrong-user');
            expect(wrongUser).toBeNull();

            const rightUser = await sharing.claimShareLink(link.uri, 'did:test:specific-user');
    ...
    describe('groups', () => {
        it('creates a group', async () => {
            const group = await sharing.createGroup({ name: 'Test Group', description: 'A test' });
        ...
        it('adds a member to a group', async () => {
            ...
            const member = await sharing.addGroupMember(group.id, 'did:test:member1', 'Alice');
        ...
        it('adds member with custom capability', async () => {
            const group = await sharing.createGroup({ name: 'Team' });
            const member = await sharing.addGroupMember(group.id, 'did:test:m1', 'Bob', 'admin');
            expect(member).not.toBeNull();
            expect(member!.capability).toBe('admin');
        });

        it('returns null adding member to nonexistent group', async () => {
            const member = await sharing.addGroupMember('fake-group', 'did:test:m', 'Nobody');
    ...
    describe('capability checking', () => {
        ...
        it('respects capability hierarchy', async () => {
            ...
            expect(sharing.hasCapability('did:test:user1', 'doc-1', 'read')).toBe(true);
            expect(sharing.hasCapability('did:test:user1', 'doc-1', 'edit')).toBe(false);
        });

        it('returns false for no grants', () => {
            expect(sharing.hasCapability('did:test:nobody', 'doc-1', 'read')).toBe(false);
        });
    });
    ...
    describe('effective capability', () => {
        it('returns highest capability for a DID', async () => {
            ...
            const effective = sharing.getEffectiveCapability('did:test:user1', 'doc-1');
            expect(effective).toBe('edit');
        });

        it('returns null for no access', () => {
            expect(sharing.getEffectiveCapability('did:test:nobody', 'doc-1')).toBeNull();
        });
    });
