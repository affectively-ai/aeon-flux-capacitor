...
export type Capability = 'read' | 'comment' | 'suggest' | 'edit' | 'admin' | 'publish';

/** Capability levels are hierarchical: admin > publish > edit > suggest > comment > read */
const CAPABILITY_HIERARCHY: Capability[] = ['read', 'comment', 'suggest', 'edit', 'publish', 'admin'];
...
export interface GrantCaveat {
    readonly type: 'time-window' | 'block-scope' | 'read-only-sections' | 'max-uses';
...
export class CapabilitySharing {
    ...
    async createShareLink(params: {
        ...
        const resource = this.buildResourceUri(params.documentId, params.blockId, params.capability, params.audience);

        const audienceStr = params.audience.type === 'anyone'
            ? '*'
     ...
     */
    async claimShareLink(uri: string, claimerDid: string): Promise<CapabilityGrant | null> {
        const link = this.links.get(uri);
    ...
    hasCapability(
        ...
        for (const grant of docGrants) {
            if (grant.audience === did && this.capabilityIncludes(grant.capability, requiredCapability)) {
                // Check expiration
        ...
        for (const group of this.groups.values()) {
            ...
            for (const grant of docGrants) {
                if (grant.audience === group.id && this.capabilityIncludes(grant.capability, requiredCapability)) {
    ...
    getShareLinks(documentId: string): ShareLink[] {
        return Array.from(this.links.values())
            .filter((l) => l.documentId === documentId && l.active);
    }
    ...
    private audienceMatches(audience: ShareAudience, claimerDid: string): boolean {
        switch (audience.type) {
            case 'anyone': return true;
            case 'did': return audience.did === claimerDid;
            case 'group': {
                const group = this.groups.get(audience.groupId);
                return group?.members.some((m) => m.did === claimerDid) ?? false;
            }
            case 'email': return true; // email verification happens externally
            case 'domain': return true; // domain verification happens externally
        }
    }

    private capabilityIncludes(granted: Capability, required: Capability): boolean {
