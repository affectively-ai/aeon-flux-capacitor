...
function makeValue(id: string, overrides: Partial<Omit<ContentValue, 'compositeValue'>> = {}): Omit<ContentValue, 'compositeValue'> {
    return {
        blockId: id,
...
function makeWeight(id: string, overrides: Partial<Omit<ContentWeight, 'compositeWeight' | 'minWeight'>> = {}): Omit<ContentWeight, 'compositeWeight' | 'minWeight'> {
    return {
        blockId: id,
        fullHeightPx: 100,
...
function makeConstraints(overrides: Partial<ContainerConstraints> = {}): ContainerConstraints {
    return {
        heightPx: 800,
        widthPx: 1200,
...
describe('ContentKnapsack', () => {
    ...
    describe('construction', () => {
        ...
        it('creates with custom value weights', () => {
            const custom = new ContentKnapsack({
                valueWeights: { emotion: 0.5, relevance: 0.2, freshness: 0.2, engagement: 0.1 },
            });
    ...
    describe('item registration', () => {
        ...
        it('registers multiple items', () => {
            for (let i = 0; i < 5; i++) {
                knapsack.registerItem(makeItem(`b${i}`), makeValue(`b${i}`), makeWeight(`b${i}`));
            }
            const result = knapsack.solve(makeConstraints());
        ...
        it('computes composite value from weights', () => {
            knapsack.registerItem(
                makeItem('b1'),
                makeValue('b1', { emotionalIntensity: 1.0, contextualRelevance: 1.0, freshness: 1.0, readerEngagement: 1.0 }),
                makeWeight('b1'),
            );
            const result = knapsack.solve(makeConstraints());
    ...
    describe('value updates', () => {
        it('updates value signals for a block', () => {
            knapsack.registerItem(makeItem('b1'), makeValue('b1', { readerEngagement: 0.1 }), makeWeight('b1'));
    ...
    describe('solving', () => {
        ...
        it('prioritizes higher-value items', () => {
            knapsack.registerItem(
                makeItem('high'),
                makeValue('high', { emotionalIntensity: 1.0, contextualRelevance: 1.0 }),
        ...
        it('preserves structural blocks', () => {
            ...
            const result = knapsack.solve(makeConstraints({ heightPx: 120, preserveStructure: true }));
            const headingDecision = result.decisions.find(d => d.blockId === 'heading');
            expect(headingDecision!.included).toBe(true);
        });

        it('respects minimum blocks constraint', () => {
            for (let i = 0; i < 5; i++) {
                knapsack.registerItem(makeItem(`b${i}`), makeValue(`b${i}`), makeWeight(`b${i}`, { fullHeightPx: 300 }));
            }
            const result = knapsack.solve(makeConstraints({ heightPx: 100, minBlocks: 3 }));
            const included = result.decisions.filter(d => d.included);
        ...
        it('enforces cognitive load budget', () => {
            ...
            const compressed = result.decisions.filter(d => d.renderMode === 'compressed');
            expect(compressed.length).toBeGreaterThan(0);
        });

        it('uses greedy solver for >100 blocks', () => {
            for (let i = 0; i < 105; i++) {
                knapsack.registerItem(makeItem(`b${i}`), makeValue(`b${i}`), makeWeight(`b${i}`, { fullHeightPx: 20 }));
            }
            const result = knapsack.solve(makeConstraints());
    ...
    describe('render modes', () => {
        it('assigns comfortable mode to highest value items', () => {
            knapsack.registerItem(
                makeItem('premium'),
                makeValue('premium', { emotionalIntensity: 1.0, contextualRelevance: 1.0, freshness: 1.0, readerEngagement: 1.0 }),
                makeWeight('premium'),
            );
            const result = knapsack.solve(makeConstraints({ heightPx: 10000 }));
        ...
        it('assigns hidden mode to overflow items', () => {
            for (let i = 0; i < 10; i++) {
                knapsack.registerItem(makeItem(`b${i}`), makeValue(`b${i}`), makeWeight(`b${i}`, { fullHeightPx: 500 }));
            }
            const result = knapsack.solve(makeConstraints({ heightPx: 300, preserveStructure: false }));
    ...
    describe('events', () => {
        it('emits layout result on solve', () => {
            ...
            knapsack.onChange((result) => { emitted = result; });
            knapsack.registerItem(makeItem('b1'), makeValue('b1'), makeWeight('b1'));
        ...
        it('supports unsubscribe', () => {
            let count = 0;
            const unsub = knapsack.onChange(() => { count++; });
            knapsack.registerItem(makeItem('b1'), makeValue('b1'), makeWeight('b1'));
            knapsack.solve(makeConstraints());
            expect(count).toBe(1);
    ...
    describe('ESI overrides', () => {
        ...
        it('applies forced render mode', () => {
            knapsack.registerItem(makeItem('b1'), makeValue('b1'), makeWeight('b1'));
            knapsack.applyESIOverrides([{ blockId: 'b1', forceRenderMode: 'collapsed' }]);
    ...
    describe('personalized solve', () => {
        ...
        it('adjusts capacity for phone device class', () => {
            const phone = knapsack.personalizedSolve(makeConstraints({ heightPx: 1000 }), {
                readerDid: 'did:test:1',
                deviceClass: 'phone',
            });
            ...
            for (let i = 0; i < 5; i++) {
                desktop.registerItem(makeItem(`b${i}`), makeValue(`b${i}`), makeWeight(`b${i}`));
            }
            const desktopResult = desktop.personalizedSolve(makeConstraints({ heightPx: 1000 }), {
                readerDid: 'did:test:1',
                deviceClass: 'desktop',
            });

            // Phone should have more compression
            expect(phone.meta.containerCapacity).toBeLessThan(desktopResult.meta.containerCapacity);
        });
