...
export type NodeShape = 'sphere' | 'cube' | 'tetrahedron' | 'torus' | 'octahedron' | 'diamond';
...
export class SpatialProjection {
    ...
    private nodeMeshes: Map<string, THREE.Mesh<THREE.BufferGeometry, THREE.MeshStandardMaterial>> = new Map();
    private edgeLines: THREE.Line<THREE.BufferGeometry, THREE.LineBasicMaterial>[] = [];
    ...
    projectDocument(
        ...
        const spatialNodes: SpatialNode[] = blocks.map((block, i) => {
            ...
            const node: SpatialNode = {
                ...
                size: this.config.nodeSize * (0.5 + block.classification.confidence * 0.5),
                opacity: 0.4 + block.classification.confidence * 0.6,
                label: block.text.length > maxLabel
    ...
    destroy(): void {
        ...
        if (this.geometries) {
            (Object.values(this.geometries) as THREE.BufferGeometry[]).forEach((g) => g.dispose());
     ...
     */
    private reduceToXYZ(embeddings: Float32Array[]): Array<[number, number, number]> {
