...
export class HypercubeProjection {
    ...
    private rotate4D(
        ...
        switch (plane) {
            case 'xw': return [x * cos - w * sin, y, z, x * sin + w * cos];
            case 'yw': return [x, y * cos - w * sin, z, y * sin + w * cos];
            case 'zw': return [x, y, z * cos - w * sin, z * sin + w * cos];
            case 'xy': return [x * cos - y * sin, x * sin + y * cos, z, w];
            case 'xz': return [x * cos - z * sin, y, x * sin + z * cos, w];
            case 'yz': return [x, y * cos - z * sin, y * sin + z * cos, w];
    ...
    private updateProjections(): void {
        for (const [, node] of this.nodes) {
            node.projected3D = this.project4Dto3D(node.position4D, this.rotationAngle);
        }
