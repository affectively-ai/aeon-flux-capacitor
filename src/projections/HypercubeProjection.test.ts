...
function makeBlock(id: string, embedding: number[], sentiment = 0): ProjectableBlock4D {
    return {
        id,
        text: `Block ${id}`,
...
describe('HypercubeProjection', () => {
    ...
    describe('4D → 3D projection', () => {
        it('stereographic produces valid 3D points', () => {
            const proj = new HypercubeProjection(makeConfig({ projectionMethod: 'stereographic' }));
        ...
        it('orthographic drops W coordinate', () => {
            const proj = new HypercubeProjection(makeConfig({ projectionMethod: 'orthographic' }));
