...
export class DocumentOracle {
    ...
    async analyze(
        ...
        const [contradictions, gaps, weaknesses, questions, toneShifts] = await Promise.all([
        ...
        insights.push(...contradictions, ...gaps, ...weaknesses, ...questions, ...toneShifts);
    ...
    async ask(
        ...
        const fullText = blocks.map((b) => `[Block ${b.id}]: ${b.text}`).join('\n\n');
            ...
            try {
                ...
                if (result.contradicts) {
                    insights.push({
                        ...
                        suggestion: 'Resolve the contradiction by clarifying or removing one of the conflicting statements.',
                        confidence: 0.7,
                        dismissed: false,
                    });
                }
            } catch { /* inference returned non-JSON */ }
        }
    ...
    private async findArgumentGaps(
      ...
      ${text}`,
        ...
        try {
            const gaps: Array<{ gap: string; afterBlockIndex: number; severity: number }> = JSON.parse(response);
            return gaps.map((g, i) => ({
                id: `gap-${i}`,
                type: 'gap' as const,
                severity: g.severity,
    ...
    private async findWeakSections(
        ...
        const text = blocks.map((b, i) => `[${i}] ${b.text}`).join('\n\n').slice(0, 3000);
    ...
    private async findUnansweredQuestions(
      ...
      ${text}`,
        ...
        try {
            const questions: Array<{ question: string; relatedBlockIndex: number }> = JSON.parse(response);
        ...
        for (let i = 1; i < blocks.length; i++) {
            ...
            if (shift > 0.5) {
                insights.push({
                    ...
                    detail: `Sentiment jumps from ${prevSentiment.toFixed(2)} to ${currSentiment.toFixed(2)} — a ${shift > 0.7 ? 'dramatic' : 'noticeable'} shift that may disorient readers.`,
                    blockIds: [blocks[i - 1].id, blocks[i].id],
                    suggestion: 'Add a transitional sentence or adjust tone for smoother flow.',
                    confidence: 0.8,
                    dismissed: false,
                });
            }
    ...
    private findRedundancies(
        ...
        for (let i = 0; i < blocks.length; i++) {
            for (let j = i + 2; j < blocks.length; j++) { // skip adjacent
                if (blocks[i].embedding && blocks[j].embedding) {
                    const sim = this.cosine(blocks[i].embedding!, blocks[j].embedding!);
                    if (sim > 0.9) {
                        insights.push({
                            ...
                            detail: `These two sections say essentially the same thing (${(sim * 100).toFixed(0)}% similarity).`,
                            blockIds: [blocks[i].id, blocks[j].id],
