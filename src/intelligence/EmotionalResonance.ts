...
export type Emotion =
    ...
    | 'curiosity' | 'confusion' | 'awe' | 'gratitude'
    | 'hope' | 'pride' | 'guilt' | 'shame'
    | 'nostalgia' | 'empowerment' | 'vulnerability'
    | 'neutral';
...
export class EmotionalResonance {
    ...
    private listeners: Set<(impacts: Map<string, EmotionalImpact>) => void> = new Set();

    constructor(config: ResonanceConfig) {
        this.config = {
            inferFn: config.inferFn,
            audiences: config.audiences ?? [
                'supporters', 'skeptics', 'newcomers', 'experts',
            ],
    ...
    async predictImpact(
        ...
        const contextStr = context
            ? `Context — preceding text: "${context.precedingText?.slice(0, 200) ?? 'n/a'}", document tone: ${context.documentTone ?? 'unknown'}, author intent: ${context.authorIntent ?? 'unknown'}`
            : '';
    ...
    async predictEditImpact(
        ...
        if (Math.abs(valenceShift) < 0.1 && Math.abs(arousalShift) < 0.1) {
            ...
            change = `More ${after.arousal > 0.6 ? 'exciting' : 'comforting'} — readers will feel more positive`;
        } else if (valenceShift < -0.2) {
            change = `More ${after.arousal > 0.6 ? 'alarming' : 'somber'} — readers will feel more negative`;
    ...
    getSummary(blockId: string): string {
        ...
        let summary = `${impact.primaryEmotion} (${(impact.intensity * 100).toFixed(0)}% intensity)`;
     ...
     */
    onChange(listener: (impacts: Map<string, EmotionalImpact>) => void): () => void {
    ...
    private heuristicImpact(blockId: string, text: string): EmotionalImpact {
        ...
        const positiveWords = new Set(['good', 'great', 'love', 'happy', 'beautiful', 'amazing', 'wonderful', 'excellent', 'best', 'hope', 'joy', 'succeed']);
        const negativeWords = new Set(['bad', 'terrible', 'hate', 'sad', 'ugly', 'awful', 'worst', 'fail', 'fear', 'anger', 'pain', 'problem']);
        ...
        const valence = length > 0 ? (posCount - negCount) / Math.max(1, posCount + negCount) : 0;
        const arousal = hasExclamation ? 0.7 : hasQuestion ? 0.5 : 0.3;

        const emotion: Emotion = valence > 0.2 ? 'joy'
            : valence < -0.2 ? 'sadness'
        ...
        return {
            ...
            secondaryEmotions: [],
            intensity: Math.min(1, (posCount + negCount) / Math.max(1, length) * 10),
            valence,
            arousal,
            dominance: 0.5,
            audienceReactions: [],
    ...
    private classifyArc(
        ...
        const avgFirst = firstThird.reduce((s, p) => s + p.valence, 0) / firstThird.length;
        const avgMiddle = middleThird.reduce((s, p) => s + p.valence, 0) / middleThird.length;
        const avgLast = lastThird.reduce((s, p) => s + p.valence, 0) / lastThird.length;

        const rising = avgLast > avgFirst + 0.2;
        const falling = avgLast < avgFirst - 0.2;
        const middlePeak = avgMiddle > avgFirst + 0.15 && avgMiddle > avgLast + 0.15;
        const middleTrough = avgMiddle < avgFirst - 0.15 && avgMiddle < avgLast - 0.15;
        ...
        const avgArousalFirst = firstThird.reduce((s, p) => s + p.arousal, 0) / firstThird.length;
        const avgArousalLast = lastThird.reduce((s, p) => s + p.arousal, 0) / lastThird.length;
    ...
    private async describeTrajectory(
        ...
        const arcDescriptions: Record<ArcType, string> = {
            'rags-to-riches': 'The document builds from tension to resolution — readers will finish feeling uplifted.',
            'riches-to-rags': 'The document starts strong but ends on a sober note — readers may feel deflated.',
            'icarus': 'The emotional peak comes mid-document; the ending may feel anticlimactic.',
            'cinderella': 'Classic story arc: tension → crisis → resolution. Emotionally satisfying.',
            'oedipus': 'Unsettling arc: starts well, dips, then rises again. Readers may feel whiplashed.',
            'steady': 'Emotionally consistent throughout. Stable, but may lack dynamic engagement.',
            'crescendo': 'Builds intensity toward the end — readers will feel the crescendo.',
            'decrescendo': 'Opens with high energy and winds down. Good for calming; risky if the ending matters.',
        };
