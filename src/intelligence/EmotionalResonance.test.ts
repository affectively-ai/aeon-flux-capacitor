...
const mockInferFn = vi.fn().mockResolvedValue(JSON.stringify({
    ...
    persuasion: 0.5,
    audienceReactions: [
        { audience: 'developers', reaction: 'interest', intensity: 0.7, reason: 'Technical content' },
    ],
    alienationRisk: null,
}));
...
describe('EmotionalResonance', () => {
    ...
    describe('construction', () => {
        ...
        it('creates with audiences', () => {
            const r = new EmotionalResonance({ inferFn: mockInferFn, audiences: ['teens', 'professionals'] });
            expect(r).toBeDefined();
        });

        it('creates with alienation checking', () => {
            const r = new EmotionalResonance({ inferFn: mockInferFn, checkAlienation: true });
            expect(r).toBeDefined();
        });
    });
    ...
    describe('predictImpact', () => {
        it('predicts emotional impact of a block', async () => {
            const impact = await resonance.predictImpact('block-0', 'A deeply moving story about courage.');
        ...
        it('falls back to heuristic on inference failure', async () => {
            ...
            const impact = await r.predictImpact('block-0', 'Test text with emotional content!');
            expect(impact).toBeDefined();
            expect(impact.blockId).toBe('block-0');
        });
    });
    ...
    describe('visualization', () => {
        it('generates summary string', async () => {
            await resonance.predictImpact('block-0', 'Exciting news about breakthroughs!');
    ...
    describe('events', () => {
        it('emits change events on predict', async () => {
            let emitted = false;
            resonance.onChange(() => { emitted = true; });
        ...
        it('supports unsubscribe', async () => {
            let count = 0;
            const unsub = resonance.onChange(() => { count++; });
            await resonance.predictImpact('block-0', 'Text.');
            expect(count).toBe(1);
