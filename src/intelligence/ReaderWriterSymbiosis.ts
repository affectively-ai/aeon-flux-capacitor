...
export class ReaderWriterSymbiosis {
    ...
    private listeners: Set<(annotations: Map<string, SymbiosisAnnotation>) => void> = new Set();
    ...
    constructor(config?: SymbiosisConfig) {
        this.config = {
            ...
            enabledSignals: config?.enabledSignals ?? [
                'slow-read', 'fast-skip', 're-read', 'drop-off',
                'highlight', 'linger', 'confusion', 'delight',
            ],
        };
    }
     ...
     */
    processReaderData(data: AggregatedReaderData[]): Map<string, SymbiosisAnnotation> {
        ...
        for (const blockData of data) {
            ...
            const timeRatio = blockData.avgTimeMs / Math.max(1, blockData.expectedTimeMs);
            ...
            if (timeRatio > 3 && this.isEnabled('slow-read')) {
                signals.push({
                    ...
                    suggestion: 'Consider simplifying this passage — readers are struggling.',
                });
            }
            ...
            if (timeRatio < 0.3 && this.isEnabled('fast-skip')) {
                signals.push({
                    ...
                    type: 'fast-skip',
                    value: 1 - timeRatio,
                    label: `${((1 - timeRatio) * 100).toFixed(0)}% of expected reading time — readers skip this`,
            ...
            if (blockData.reReadRate > 0.2 && this.isEnabled('re-read')) {
                signals.push({
                    ...
                    label: `${(blockData.reReadRate * 100).toFixed(0)}% of readers re-read this`,
                    color: 'hsla(50, 80%, 55%, 0.6)',
                    intensity: blockData.reReadRate > 0.4 ? 'strong' : 'medium',
                    suggestion: blockData.reReadRate > 0.4
            ...
            if (blockData.dropOffRate > 0.15 && this.isEnabled('drop-off')) {
                signals.push({
                    ...
                    label: `${(blockData.dropOffRate * 100).toFixed(0)}% of readers leave here`,
                    color: 'hsla(0, 80%, 55%, 0.6)',
                    intensity: blockData.dropOffRate > 0.3 ? 'strong' : 'medium',
                    suggestion: 'Major drop-off point. Consider restructuring to maintain momentum.',
                });
            }
            ...
            if (blockData.highlightRate > 0.1 && this.isEnabled('highlight')) {
                signals.push({
                    ...
                    label: `${(blockData.highlightRate * 100).toFixed(0)}% of readers highlight this`,
            ...
            if (blockData.shareRate > 0.05 && this.isEnabled('share')) {
                signals.push({
                    ...
                    label: `${(blockData.shareRate * 100).toFixed(0)}% of readers share from here`,
            ...
            if (timeRatio > 2 && blockData.reReadRate > 0.3 && this.isEnabled('confusion')) {
                signals.push({
                    blockId: blockData.blockId,
                    type: 'confusion',
                    ...
                    intensity: 'strong',
                    suggestion: 'This is your most confusing passage. Rewrite or add context.',
                });
            }

            // Compound signals: delight = highlight + share + linger
            if (blockData.highlightRate > 0.1 && blockData.shareRate > 0.03 && this.isEnabled('delight')) {
                signals.push({
                    blockId: blockData.blockId,
                    type: 'delight',
    ...
    getSummary(): string {
        ...
        if (attention.length === 0) {
            return `Readers are engaging well across all sections (${(score * 100).toFixed(0)}% experience score).`;
        }

        const worst = attention[0];
        const worstSignal = worst.signals.reduce((a, b) => a.value > b.value ? a : b);

        return `${attention.length} section${attention.length > 1 ? 's' : ''} need attention. ` +
     ...
     */
    onChange(listener: (annotations: Map<string, SymbiosisAnnotation>) => void): () => void {
