...
export class SemanticBacklinks {
    private config: Required<Omit<BacklinkConfig, 'queryCorpus'>> & { queryCorpus?: BacklinkConfig['queryCorpus'] };
    private index: BacklinkIndex = {
        incoming: new Map(),
        outgoing: new Map(),
    };
    private explicitLinks: Map<string, Set<string>> = new Map();
    private listeners: Set<(blockId: string, backlinks: Backlink[]) => void> = new Set();
    ...
    indexDocument(
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            for (let j = 0; j < blocks.length; j++) {
                ...
                const sim = this.cosineSimilarity(blocks[i].embedding, blocks[j].embedding);
                ...
                const linkType: BacklinkType = sharedEntities.length > 0
                    ? 'entity-coref'
                    : Math.abs(i - j) === 1
                        ? 'continuation'
                        : 'semantic';
     ...
     */
    addExplicitLink(fromBlockId: string, toBlockId: string, sourceDoc: {
        documentId: string;
        documentTitle: string;
        excerpt: string;
    }): void {
     ...
     */
    onChange(listener: (blockId: string, backlinks: Backlink[]) => void): () => void {
