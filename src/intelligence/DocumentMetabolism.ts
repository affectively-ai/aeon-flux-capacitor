...
export interface StalenessIndicator {
    ...
    readonly type: 'version-reference' | 'date-reference' | 'api-reference' | 'link-broken' | 'fact-check' | 'opinion-dated';
...
export class DocumentMetabolism {
    ...
    private static readonly STALENESS_PATTERNS: Array<{
        ...
        reasonFn: (match: RegExpMatchArray) => string;
    }> = [
            {
                ...
                reasonFn: (m) => `References version ${m[0]} — check if this is still current`,
            },
            {
                pattern: /(?:as of|since|in)\s+(20\d{2}|January|February|March|April|May|June|July|August|September|October|November|December)\b/gi,
                type: 'date-reference',
            ...
            },
            {
                ...
                type: 'api-reference',
                reasonFn: (m) => `Contains "${m[0]}" — likely references something no longer current`,
            },
            {
                pattern: /https?:\/\/[^\s)]+/gi,
                type: 'link-broken',
    ...
    async analyze(
        ...
        const evergreenBlocks = freshnessResults.filter((f) => f.lifespan === 'evergreen').length;
        const overallFreshness = freshnessResults.length > 0
            ? freshnessResults.reduce((s, f) => s + f.freshness, 0) / freshnessResults.length
            : 1;

        // Estimate document half-life
        const lifespanDays = freshnessResults.map((f) => this.lifespanToDays(f.lifespan));
        const avgLifespan = lifespanDays.reduce((s, d) => s + d, 0) / Math.max(1, lifespanDays.length);
        ...
        const refreshQueue = freshnessResults
            ...
            .map((f) => ({
                ...
                reason: f.refreshAction ?? f.indicators[0]?.reason ?? 'Content may be stale',
            }))
    ...
    getSummary(): string {
        ...
        return `${h.staleBlocks} of ${h.totalBlocks} sections need refresh. ` +
    ...
    private async analyzeBlock(block: {
        ...
        if (this.config.inferFn && block.text.length > 20) {
            ...
            lifespan = this.heuristicLifespan(block.text, block.blockType, indicators);
        }
        ...
        const indicatorPenalty = indicators.reduce((s, i) => s + i.severity * 0.1, 0);
        const freshness = Math.max(0, Math.min(1, decayFactor - indicatorPenalty));

        const needsRefresh = freshness < 0.5 || indicators.some((i) => i.severity > 0.7);

        const predictedExpiry = lifespan === 'evergreen'
            ? null
            : new Date(new Date(block.lastEdited).getTime() + halfLifeDays * 24 * 60 * 60 * 1000).toISOString();

        const daysUntilStale = predictedExpiry
            ? (new Date(predictedExpiry).getTime() - Date.now()) / (24 * 60 * 60 * 1000)
            : Infinity;
        ...
        for (const { pattern, type, reasonFn } of allPatterns) {
            ...
            while ((match = regex.exec(text)) !== null) {
                indicators.push({
                    ...
                    severity: type === 'link-broken' ? 0.3
                        : type === 'version-reference' ? 0.6
                            : type === 'date-reference' ? 0.5
                                : type === 'api-reference' ? 0.7
                                    : 0.4,
                });
            }
        }
    ...
    private heuristicLifespan(
        ...
        if (indicators.some((i) => i.type === 'version-reference')) return 'transient';
    ...
    private async classifyLifespan(text: string, blockType: string): Promise<ContentLifespan> {
      ...
      Text: "${text.slice(0, 300)}"`,
        ...
        if (['ephemeral', 'transient', 'durable', 'evergreen'].includes(normalized)) {
    ...
    private lifespanToDays(lifespan: ContentLifespan): number {
        switch (lifespan) {
            ...
            case 'transient': return 90;
            case 'durable': return 365;
            case 'evergreen': return 3650; // 10 years
            case 'unknown': return this.config.defaultHalfLife ?? 180;
        }
    }
