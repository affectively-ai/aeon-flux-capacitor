...
function makeReaderData(blockId: string, overrides: Partial<AggregatedReaderData> = {}): AggregatedReaderData {
    return {
        blockId,
        avgTimeMs: 5000,
        expectedTimeMs: 4000,
...
describe('ReaderWriterSymbiosis', () => {
    ...
    describe('processReaderData', () => {
        ...
        it('detects slow reading (spending significantly more time than expected)', () => {
            const data = [makeReaderData('b0', { avgTimeMs: 20000, expectedTimeMs: 4000 })];
            const annotations = symbiosis.processReaderData(data);
            const ann = annotations.get('b0');
            expect(ann).toBeDefined();
        });
        ...
        it('marks blocks needing attention', () => {
            ...
            const data = [makeReaderData('b0', { avgTimeMs: 20000, expectedTimeMs: 4000, dropOffRate: 0.7, reReadRate: 0.6 })];
            const annotations = symbiosis.processReaderData(data);
            const ann = annotations.get('b0');
    ...
    describe('getAttentionBlocks', () => {
        it('returns blocks sorted by urgency', () => {
            symbiosis.processReaderData([
                makeReaderData('b0', { avgTimeMs: 15000, expectedTimeMs: 4000, dropOffRate: 0.7, reReadRate: 0.5 }),
                makeReaderData('b1', { dropOffRate: 0.1 }),
                makeReaderData('b2', { avgTimeMs: 25000, expectedTimeMs: 4000, dropOffRate: 0.9, reReadRate: 0.6 }),
            ]);
    ...
    describe('events', () => {
        it('emits on data processing', () => {
            let emitted = false;
            symbiosis.onChange(() => { emitted = true; });
            symbiosis.processReaderData([makeReaderData('b0')]);
        ...
        it('supports unsubscribe', () => {
            let count = 0;
            const unsub = symbiosis.onChange(() => { count++; });
            symbiosis.processReaderData([makeReaderData('b0')]);
            expect(count).toBe(1);
            unsub();
