...
function makeBlocksWithEmbeddings(count = 3) {
    return makeBlocks(count).map((b, i) => ({
        ...b,
        embedding: new Float32Array(Array.from({ length: 64 }, (_, j) => Math.sin(i + j * 0.1))),
    }));
}
...
describe('DocumentOracle', () => {
    ...
    describe('construction', () => {
        ...
        it('creates with optional thresholds', () => {
            const o = new DocumentOracle({ inferFn: mockInferFn, severityThreshold: 0.5, maxInsights: 10 });
    ...
    describe('buildProfile', () => {
        it('builds a document profile', async () => {
            mockInferFn.mockResolvedValueOnce(JSON.stringify({
                qualityScore: 0.8,
                coherence: 0.7,
                readingLevel: 12,
                audience: 'developers',
    ...
    describe('events', () => {
        ...
        it('supports unsubscribe', async () => {
            let count = 0;
            const unsub = oracle.onChange(() => { count++; });
            await oracle.analyze(makeBlocks());
            expect(count).toBe(1);
