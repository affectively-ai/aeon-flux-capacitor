...
describe('DocumentMetabolism', () => {
    ...
    describe('construction', () => {
        ...
        it('creates with inferFn', () => {
            const m = new DocumentMetabolism({ inferFn: vi.fn().mockResolvedValue('ephemeral') });
        ...
        it('creates with custom staleness patterns', () => {
            const m = new DocumentMetabolism({
                customPatterns: [
                    { pattern: /React\s+\d+/, type: 'version-reference', reason: 'React version reference' },
                ],
            });
            expect(m).toBeDefined();
        });
    ...
    describe('staleness detection', () => {
        ...
        it('detects API references that may be outdated', async () => {
            const blocks = makeBlocks(1, 30);
            blocks[0].text = 'Use api.example.com/v2/users endpoint for authentication.';
            await metabolism.analyze(blocks);
            const freshness = metabolism.getBlockFreshness('block-0');
    ...
    describe('lifespan classification', () => {
        it('classifies heading as durable (structural content)', async () => {
            const blocks = [{ id: 'h1', text: 'Getting Started', lastEdited: new Date().toISOString(), blockType: 'heading' }];
            await metabolism.analyze(blocks);
        ...
        it('classifies version-specific content as shorter-lived', async () => {
            const blocks = [{ id: 'b1', text: 'React 18.2.0 introduces concurrent rendering for better UX.', lastEdited: new Date().toISOString(), blockType: 'paragraph' }];
            await metabolism.analyze(blocks);
            const freshness = metabolism.getBlockFreshness('b1');
            // Content with version references should have a shorter lifespan
            expect(['ephemeral', 'seasonal', 'durable', 'evergreen']).toContain(freshness!.lifespan);
        });
    });
    ...
    describe('events', () => {
        it('emits health change events', async () => {
            let emitted = false;
            metabolism.onChange(() => { emitted = true; });
            await metabolism.analyze(makeBlocks());
        ...
        it('supports unsubscribe', async () => {
            let count = 0;
            const unsub = metabolism.onChange(() => { count++; });
            await metabolism.analyze(makeBlocks());
            expect(count).toBe(1);
            unsub();
            await metabolism.analyze(makeBlocks());
