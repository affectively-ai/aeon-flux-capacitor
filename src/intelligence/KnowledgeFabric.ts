...
export class KnowledgeFabric {
    private config: Required<Omit<FabricConfig, 'inferFn'>> & { inferFn?: (prompt: string) => Promise<string> };
    private suggestions: Map<string, FabricSuggestion[]> = new Map(); // blockId → suggestions
    private debounceTimers: Map<string, ReturnType<typeof setTimeout>> = new Map();
    private listeners: Set<(blockId: string, suggestions: FabricSuggestion[]) => void> = new Set();
    ...
    async fetchSuggestions(
        ...
        for (const result of results) {
            const relationship = await this.classifyRelationship(text, result.text, result.similarity);
     ...
     */
    onChange(listener: (blockId: string, suggestions: FabricSuggestion[]) => void): () => void {
    ...
    private async classifyRelationship(
      ...
      Source material: "${sourceText.slice(0, 300)}"`,
        ...
        const valid: FabricRelationship[] = [
            'supporting', 'contradicting', 'extending', 'prior-art',
            'reference', 'parallel', 'foundation',
        ];
    ...
    private suggestUsage(relationship: FabricRelationship): FabricUsage {
        switch (relationship) {
            case 'supporting': return 'cite';
            case 'contradicting': return 'refute';
            case 'extending': return 'link';
            case 'prior-art': return 'merge';
            case 'reference': return 'cite';
            case 'parallel': return 'link';
            case 'foundation': return 'absorb';
        }
    }
