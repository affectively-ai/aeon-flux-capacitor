...
describe('KnowledgeFabric', () => {
    ...
    describe('construction', () => {
        ...
        it('creates with inferFn', () => {
            const f = new KnowledgeFabric(makeConfig({
                inferFn: vi.fn().mockResolvedValue(JSON.stringify({ relationship: 'supports' })),
            }));
    ...
    describe('fetchSuggestions', () => {
        ...
        it('classifies relationship type', async () => {
            const suggestions = await fabric.fetchSuggestions(
                'block-0', 'Text', new Float32Array([0.1]), 'doc-1',
            );
    ...
    describe('getSuggestions', () => {
        it('returns cached suggestions for a block', async () => {
            await fabric.fetchSuggestions('block-0', 'Text', new Float32Array([0.1]), 'doc-1');
    ...
    describe('getAllSuggestions', () => {
        it('returns all suggestions across blocks', async () => {
            await fabric.fetchSuggestions('b0', 'Text 0', new Float32Array([0.1]), 'doc-1');
            await fabric.fetchSuggestions('b1', 'Text 1', new Float32Array([0.2]), 'doc-1');
    ...
    describe('useSuggestion', () => {
        it('marks a suggestion as used', async () => {
            const suggestions = await fabric.fetchSuggestions('b0', 'Text', new Float32Array([0.1]), 'doc-1');
    ...
    describe('getUnseenCount', () => {
        it('counts unseen suggestions', async () => {
            await fabric.fetchSuggestions('b0', 'Text', new Float32Array([0.1]), 'doc-1');
    ...
    describe('surfaceForBlock (debounced)', () => {
        it('calls corpus search after debounce', async () => {
            const debounced = new KnowledgeFabric(makeConfig({ debounceMs: 10, autoSurface: true }));
    ...
    describe('events', () => {
        ...
        it('supports unsubscribe', async () => {
            let count = 0;
            const unsub = fabric.onChange(() => { count++; });
            await fabric.fetchSuggestions('b0', 'Text', new Float32Array([0.1]), 'doc-1');
            expect(count).toBe(1);
            unsub();
            await fabric.fetchSuggestions('b1', 'Text', new Float32Array([0.1]), 'doc-1');
            expect(count).toBe(1);
        });
    });
