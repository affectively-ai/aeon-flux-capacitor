...
export interface CodeSymbol {
    ...
    /** Symbol type */
    readonly kind: 'function' | 'class' | 'method' | 'variable' | 'block' | 'unknown';
...
export class CodeRuntime {
     ...
     */
    analyzeBlock(blockId: string, code: string, declaredLanguage?: string): CodeBlockMeta {
    ...
    private looksExecutable(code: string, language: string): boolean {
        const nonExecutable = ['json', 'yaml', 'toml', 'xml', 'html', 'css', 'markdown', 'text'];
...
const CODE_ACTIONS: CodeAction[] = [
    ...
    },
    {
        ...
        icon: '🧪',
        applicable: (meta) => meta.isExecutable && meta.symbols.some((s) => s.kind === 'function'),
    },
    {
    ...
    },
    {
        ...
        icon: '📐',
        applicable: (meta) =>
            meta.isExecutable && ['javascript', 'python', 'unknown'].includes(meta.language),
    },
];
