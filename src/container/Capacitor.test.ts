...
import type { CapacitorConfig, CapacitorBlock, CapacitorEvent } from './Capacitor';
...
function makeBlock(id: string, overrides: Partial<CapacitorBlock> = {}): CapacitorBlock {
    return {
        id,
...
describe('Capacitor', () => {
    ...
    describe('construction', () => {
        ...
        it('creates with ESI config', () => {
            const withEsi = new Capacitor(makeConfig({
    ...
    describe('block registration', () => {
        ...
        it('runs inference when inferFn is provided', async () => {
            const inferFn = vi.fn().mockResolvedValue('{"valence":0.7,"arousal":0.5,"dominance":0.6,"sentiment":0.8}');
            const capWithInfer = new Capacitor(makeConfig({ inferFn, autoSolve: false }));
        ...
        it('runs embedding when embedFn is provided', async () => {
            ...
            const capWithEmbed = new Capacitor(makeConfig({ embedFn, autoSolve: false }));
    ...
    describe('projection switching', () => {
        it('switches projection and emits event', () => {
            ...
            const projEvent = events.find(e => e.type === 'projection-changed') as any;
    ...
    describe('height estimation', () => {
        it('estimates height for different block types', async () => {
            await cap.addBlock(makeBlock('heading', { type: 'heading', text: 'Title' }));
            await cap.addBlock(makeBlock('code', { type: 'code', text: 'const x = 1;' }));
            await cap.addBlock(makeBlock('image', { type: 'image', text: 'alt text' }));
            await cap.addBlock(makeBlock('table', { type: 'table', text: 'a|b|c' }));
            await cap.addBlock(makeBlock('list', { type: 'list', text: '- item 1\n- item 2' }));
            await cap.addBlock(makeBlock('quote', { type: 'blockquote', text: 'A quote.' }));
