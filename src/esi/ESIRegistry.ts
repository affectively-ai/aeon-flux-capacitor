...
export class ESIRegistry {
    ...
    async invoke(
        ...
        try {
            ...
            const result =
                data.choices?.[0]?.message?.content || data.result || JSON.stringify(data);
    ...
    private registerBuiltinTags(): void {
        ...
        this.register({
            name: 'voice-analyze',
            label: 'Analyze Voice',
            description: 'Analyze writing tone, style, and voice characteristics from embeddings',
            display: 'block',
        ...
        this.register({
            ...
            description: 'Generate text matching a trained voice/tone model derived from embeddings',
            display: 'block',
