...
describe('TemporalNavigation', () => {
    ...
    describe('snapshot recording', () => {
        ...
        it('records with document embedding', () => {
            ...
            const snapshot = nav.recordSnapshot('did:test:1', makeBlocks(), embedding);
            expect(snapshot.documentEmbedding).toBeDefined();
        });

        it('records with revision label', () => {
            const snapshot = nav.recordSnapshot('did:test:1', makeBlocks(), undefined, undefined, 'v1.0');
    ...
    describe('embedding distance', () => {
        it('computes distance between embeddings', () => {
            const s1 = nav.recordSnapshot('did:test:1', makeBlocks(), new Float32Array([1, 0, 0]));
            const s2 = nav.recordSnapshot('did:test:1', makeBlocks(), new Float32Array([0, 1, 0]));
