...
export interface TemporalCurve {
    ...
    readonly metric: 'sentiment' | 'confidence' | 'complexity' | 'similarity-to-final';
...
export class TemporalNavigation {
    ...
    private listeners: Set<(snapshot: TemporalSnapshot, index: number, total: number) => void> = new Set();
     ...
     */
    getBlockAtTime(blockId: string, timestamp: string): TemporalBlockState | null {
    ...
    diffSnapshots(fromId: string, toId: string): TemporalDiff | null {
        ...
        for (const [id, fromState] of from.blockStates) {
            ...
            if (fromState.text !== toState.text) {
                textChanged.push({ blockId: id, from: fromState.text, to: toState.text });
            }

            if (fromState.embedding && toState.embedding) {
                const distance = this.embeddingDistance(fromState.embedding, toState.embedding);
                if (distance > 0.1) {
            ...
            if (blockId) {
                ...
                switch (metric) {
                    ...
                    case 'similarity-to-final': {
                        ...
                        if (blockState.embedding && finalState?.embedding) {
                            value = 1 - this.embeddingDistance(blockState.embedding, finalState.embedding);
                        }
                        break;
                    }
                }
                ...
                switch (metric) {
                    case 'sentiment':
                        ...
                        break;
                    case 'similarity-to-final':
                        if (snapshot.documentEmbedding && finalSnapshot?.documentEmbedding) {
                            value = 1 - this.embeddingDistance(snapshot.documentEmbedding, finalSnapshot.documentEmbedding);
                        }
                        break;
                    case 'complexity': {
                        let totalWords = 0;
    ...
    getTimeline(): Array<{
        ...
        return this.snapshots.map((s) => ({
            ...
            blocksModified: Array.from(s.blockStates.values()).filter((b) => b.modified).length,
        }));
    }
    ...
    getPosition(): { index: number; total: number; progress: number } {
        return {
            ...
            progress: this.snapshots.length > 1
                ? this.currentIndex / (this.snapshots.length - 1)
                : 0,
        };
    }
     ...
     */
    onScrub(listener: (snapshot: TemporalSnapshot, index: number, total: number) => void): () => void {
