...
const DEFAULT_ACTIONS: WordAction[] = [
    // AI actions
    { id: 'define', label: 'Define', icon: '📖', category: 'ai', requiresESI: true },
    { id: 'synonyms', label: 'Synonyms', icon: '🔄', category: 'ai', requiresESI: true },
    { id: 'translate', label: 'Translate', icon: '🌐', category: 'ai', requiresESI: true },
    { id: 'rewrite', label: 'Rewrite', icon: '✨', category: 'ai', requiresESI: true },
    { id: 'expand', label: 'Expand', icon: '📝', category: 'ai', requiresESI: true },
    { id: 'tone', label: 'Change Tone', icon: '🎭', category: 'ai', requiresESI: true },
    { id: 'analyze', label: 'Analyze', icon: '🔬', category: 'ai', requiresESI: true },
...
export const WordActionMenu: React.FC<WordActionMenuProps> = ({
    ...
    const visibleActions = DEFAULT_ACTIONS.filter((action) => {
        if (readOnly && (action.category === 'edit' || action.category === 'format')) {
            return false;
        }
        ...
        >
            ...
            <div className="afc-word-action-tabs" role="tablist">
                <button
                    ...
                    className={`afc-word-action-tab ${category === null ? 'afc-active' : ''}`}
                ...
                {categories.map((cat) => (
                    <button
                        ...
                        className={`afc-word-action-tab ${category === cat ? 'afc-active' : ''}`}
        ...
        >
            {!readOnly && (
                <div className="afc-toolbar-group">
                    <button className="afc-fmt-btn" onClick={() => onFormat('bold')} title="Bold (Ctrl+B)">
                        <strong>B</strong>
                    </button>
                    <button className="afc-fmt-btn" onClick={() => onFormat('italic')} title="Italic (Ctrl+I)">
                        <em>I</em>
                    </button>
                    <button className="afc-fmt-btn" onClick={() => onFormat('code')} title="Code (Ctrl+`)">
                        {'</>'}
                    </button>
                    <button className="afc-fmt-btn" onClick={() => onFormat('link')} title="Link (Ctrl+K)">
                        🔗
                    </button>
                    <button className="afc-fmt-btn" onClick={() => onFormat('highlight')} title="Highlight">
                        🖊️
                    </button>
                    <button className="afc-fmt-btn" onClick={() => onFormat('strikethrough')} title="Strikethrough">
                        <s>S</s>
                    </button>
                </div>
            )}
