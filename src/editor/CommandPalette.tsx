...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
    ...
    const allItems = useMemo(() => {
        ...
        // View
        items.push(
            { id: 'view-edit', label: 'Edit Mode', category: 'view', icon: '✎', shortcut: 'Ctrl+Shift+V', action: () => setViewMode('edit') },
            { id: 'view-markdown', label: 'Markdown Mode', category: 'view', icon: '⌘', action: () => setViewMode('markdown') },
            { id: 'view-preview', label: 'Preview Mode', category: 'view', icon: '◎', action: () => setViewMode('preview') },
            { id: 'view-spatial', label: 'Spatial Mode', category: 'view', icon: '⬡', action: () => setViewMode('spatial') },
            { id: 'fullscreen', label: 'Toggle Fullscreen', category: 'view', icon: '⊡', shortcut: 'Ctrl+Shift+F', action: toggleFullscreen },
        );

        // Document
        items.push(
            { id: 'doc-export-md', label: 'Export as Markdown', category: 'document', icon: '📄', action: () => { } },
            { id: 'doc-export-json', label: 'Export as JSON', category: 'document', icon: '{ }', action: () => { } },
            { id: 'doc-stats', label: 'Document Statistics', category: 'document', icon: '📊', action: () => { } },
        );
        ...
        return items;
    }, [doc, embeddingDoc, entities, revisions, extraItems, setViewMode, toggleFullscreen]);
                        ...
                        <div key={category} className="afc-palette-group">
                            ...
                            {items.map((item) => {
                                ...
                                return (
                                    <button
                                        key={item.id}
                                        className={`afc-palette-item ${globalIdx === selectedIndex ? 'afc-selected' : ''}`}
                                    ...
                                    >
                                        ...
                                        <div className="afc-palette-item-text">
                                            <span className="afc-palette-item-label">{item.label}</span>
                                            {item.subtitle && (
                                                <span className="afc-palette-item-subtitle">{item.subtitle}</span>
                                            )}
                                        </div>
                                        {item.shortcut && (
                                            <kbd className="afc-palette-item-shortcut">{item.shortcut}</kbd>
                                        )}
                                    </button>
                                );
                            })}
