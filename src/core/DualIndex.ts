...
export interface SomaticMarker {
    /** Marker type */
    readonly type: 'tension' | 'warmth' | 'heaviness' | 'lightness' | 'constriction' | 'expansion' | 'energy' | 'calm';
    /** Intensity (0-1) */
    readonly intensity: number;
    /** Body region (for spatial mapping) */
    readonly region?: 'chest' | 'throat' | 'gut' | 'head' | 'limbs' | 'whole-body';
}
...
export class DualIndex {
    ...
    private listeners: Set<(blockId: string, sample: RenderSample) => void> = new Set();
    ...
    queryAmygdala(filter: {
        ...
        for (const entry of this.amygdala.values()) {
            ...
            if (filter.minIntensity !== undefined && entry.intensity < filter.minIntensity) continue;
            if (filter.minValence !== undefined && entry.valence < filter.minValence) continue;
            if (filter.maxValence !== undefined && entry.valence > filter.maxValence) continue;
            results.push(entry);
        }
     ...
     */
    onSample(listener: (blockId: string, sample: RenderSample) => void): () => void {
    ...
    getStats(): {
        ...
        let bothIndexed = 0, amygdalaOnly = 0, hippocampusOnly = 0;
    ...
    private interpolate(a: AmygdalaEntry, h: HippocampusEntry): InterpolatedSample {
    ...
    private emotionToHue(emotion: string): number {
        const hueMap: Record<string, number> = {
            joy: 50, sadness: 220, anger: 0, fear: 270,
            surprise: 35, disgust: 90, trust: 170, anticipation: 30,
            curiosity: 190, confusion: 240, awe: 280, gratitude: 140,
            hope: 160, pride: 45, guilt: 300, shame: 310,
            nostalgia: 35, empowerment: 55, vulnerability: 250,
            neutral: 200,
        };
    ...
    private heuristicAmygdala(blockId: string, text: string): AmygdalaEntry {
        ...
        const positiveWords = new Set(['love', 'great', 'amazing', 'beautiful', 'wonderful', 'happy', 'joy', 'hope']);
        const negativeWords = new Set(['hate', 'terrible', 'awful', 'sad', 'angry', 'fear', 'pain', 'suffer']);
