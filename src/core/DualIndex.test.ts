...
function makeAmygdala(blockId: string, overrides: Partial<AmygdalaEntry> = {}): AmygdalaEntry {
    return {
        blockId,
        valence: 0.6,
        arousal: 0.4,
...
function makeHippocampus(blockId: string, overrides: Partial<HippocampusEntry> = {}): HippocampusEntry {
    return {
        blockId,
...
describe('DualIndex', () => {
    ...
    describe('sampling', () => {
        ...
        it('returns a RenderSample with interpolated values', () => {
            index.setAmygdala(makeAmygdala('block-1', { valence: 0.8, arousal: 0.6 }));
            index.setHippocampus(makeHippocampus('block-1', { embedding: new Float32Array([0.5, 0.5]) }));
        ...
        it('blends differently at ratio 0 (all context)', () => {
            ...
            contextOnly.setAmygdala(makeAmygdala('block-1', { valence: 1.0, arousal: 1.0 }));
        ...
        it('blends differently at ratio 1 (all emotion)', () => {
            ...
            emotionOnly.setAmygdala(makeAmygdala('block-1', { valence: 1.0, arousal: 1.0 }));
    ...
    describe('querying', () => {
        it('queries amygdala by emotion', () => {
            ...
            index.setAmygdala(makeAmygdala('b2', { emotion: 'sadness', intensity: 0.6 }));
        ...
        it('queries amygdala by intensity', () => {
            index.setAmygdala(makeAmygdala('b1', { emotion: 'joy', intensity: 0.8 }));
            index.setAmygdala(makeAmygdala('b2', { emotion: 'sadness', intensity: 0.3 }));
    ...
    describe('cross-document links', () => {
        it('stores cross-doc links in hippocampus entries', () => {
            const links = [
                { documentId: 'doc-2', blockId: 'intro', similarity: 0.85, relationship: 'supports' },
            ];

            index.setAmygdala(makeAmygdala('block-1'));
            index.setHippocampus(makeHippocampus('block-1', { crossDocLinks: links }));
    ...
    describe('listeners', () => {
        ...
        it('supports unsubscribe', () => {
            let count = 0;
            const unsub = index.onSample(() => { count++; });
