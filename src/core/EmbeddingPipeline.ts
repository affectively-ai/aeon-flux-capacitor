...
export class EmbeddingPipeline {
    ...
    private pendingTimers: Map<string, ReturnType<typeof setTimeout>> =
        new Map();
    ...
    async embedAll(doc: EmbeddingDocument): Promise<void> {
        ...
        for (let i = 0; i < nodes.length; i += batchSize) {
            ...
            await Promise.all(
                batch.map((node) => this.processNode(doc, node.id)),
            );
        }
    }
    ...
    private linkEntities(
        ...
        return entities.map((entity) => {
            ...
            const canonicalId = `entity-${entity.type}-${entity.text.toLowerCase().replace(/\s+/g, '-')}`;
            return { ...entity, canonicalId };
        });
    }
}
